...
router.route("/update/:id").post((req, res) => {
  Expense.findById(req.params.id)
    .then((expense) => {
      ...
      if (req.body.date) {
        expense.date = dateTime.create(req.body.date).format();
      }
...
router.route('/category/:category').get((req,res) => {
    Expense.find({category: req.params.category}).then(expenses => res.send(expenses)).catch(err => res.status(400).json('Error: '+er))
})
