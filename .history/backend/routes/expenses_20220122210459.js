...
router.route("/update/:id").post((req, res) => {
  Expense.findById(req.params.id)
    .then((expense) => {
      ...
      expense.date = dateTime.create(req.body.date).format("Y/n/d H:M:S");
      expense.year = Number(expense.date.slice(0, 4));
      expense.month = expense.date.slice(5, 8);
      expense.day = Number(expense.date.slice(9, 11));
...
router.route("/search/date/:date").get((req, res) => {
  Expense.find({ date: new Date(req.params.date) })
    .then((expenses) => res.json(expenses))
    .catch((err) => res.status(400).json(err));
});*/
