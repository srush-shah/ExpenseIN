...
router.route("/add").post((req, res) => {
  ...
  const date = dateTime.create(req.body.date).format('Y/n/d H');

  const dtepoch = new Date(req.params.body) | Date.now();
  /*const year = dtepoch.getFullYear();
  const month = ("0" + (dtepoch.getMonth() + 1)).slice(-2);
  const day = ("0" + dtepoch.getDay()).slice(-2);*/
